...
import { setRating } from '../../../slices/rating';
import style from './Star.module.css';

export default function Star({ isFilled, serial }) {
  const initClasses = style.star;
  const [classes, setClasses] = useState(initClasses);
  const dispatch = useDispatch();

  useEffect(() => {
    isFilled ? setClasses(initClasses + ' ' + style.filled) : setClasses(initClasses);
  }, [initClasses, isFilled]);

  const onClickStar = () => dispatch(setRating(serial));
